...
=== CORE OUTPUT RULES ===
- Return 1 to 2 Creative Type labels only.
- Use exact spelling from the allowed list.
- Choose labels based on the MAIN visual/content focus, not just the song.
- If isSlideshow=true, ALWAYS include Carousel as one Creative Type.
- If the post is ambiguous, choose the strongest visible signal and lower confidence.
- Do not use Remix for visual transitions or fashion reworks. Remix is audio-only.

=== NARRATIVE RULES (flexible short theme) ===
- Narrative is NOT a fixed category. Write a concise theme phrase in 1 to 5 words.
- Use a common short theme when it fits, but create a custom phrase when needed.
- Do NOT force every post into Relationship / Dance / Lifestyle.
- Use NA only when no meaningful narrative or theme is visible.
- Good examples: Relationship, Girl lip syncing, Simple dance, Dance tutorial, Beach vacation, Makeup routine, Product showcase, Rainy walk, Healing, Study motivation, Celebrity edit, Drama edit, Gaming clip, Ramadan prayer, Outfit inspiration, Travel vlog, Daily routine.
- For romance/longing/love: Relationship, Late night conversation, Missing someone, Couple memory.
- For daily/aesthetic scenes: Daily routine, Rainy walk, Campus life, Night drive, Slice of life.
- For beauty/fashion/tutorials: Makeup routine, Skincare products, Hairstyle transformation, Outfit showcase, Fit check, Product showcase.
- For KR/PH/SG style edits: Simple dance, Idol edit, Celebrity edit, Drama edit, Game edit.

=== CREATIVE TYPE HANDBOOK ===

1) Carousel
Use when the post is a photo slideshow or image carousel. If isSlideshow=true, include Carousel even when another theme is also present.
Examples: skincare product carousel -> Beauty, Carousel; quote photo carousel -> Quotes, Carousel; outfit photo carousel -> Fashion, Carousel.

2) Fashion
Use when the main focus is clothing, styling, OOTD, fit check, lookbook, outfit transition, clothing haul, accessories, or showing an outfit from different angles.
Do NOT use Fashion when the main focus is makeup, skincare, hair, or nails. Use Beauty instead.
Example: a girl showcasing her outfit from different angles -> Fashion.

3) Beauty
Use when the main focus is makeup, skincare, hair, nails, cosmetics, beauty products, makeup routine, GRWM focused on makeup, skincare routine, eye makeup, hair tutorial, hairstyle transformation, nail art.
Do NOT use Beauty just because the person looks attractive. If clothing/styling is the focus -> Fashion.
Examples: makeup routine -> Beauty; skincare product carousel -> Beauty, Carousel; hairstyle transformation -> Beauty; nail art -> Beauty.

4) Relationship
Use when the main message is romantic love, affection, longing, heartbreak, devotion, partner dynamics, boyfriend/girlfriend, husband/wife, missing someone, or talking about someone special.
Examples: affection for candid photos taken by someone special -> Relationship; late-night talking to someone special -> Relationship; patience, longing and devotion -> Relationship; emotional openness between partners -> Relationship.
Do NOT use Relationship for ordinary daily memories unless romance is the clear main message.

5) Slice of Life
Use for ordinary everyday moments, lifestyle, daily routines, walking, studying, commuting, coffee, home life, rain scenes, casual memories, aesthetic personal moments.
If the post documents life/memories without a clear romantic message -> Slice of Life.
If romance is the main emotional message -> Relationship.

6) Dance
Use when dance performance, choreography, dance challenge, hand gesture dance, group dance, dance tutorial, or dance trend is the main focus.
Do NOT use Dance when the person is mainly singing/mouthing lyrics. Use Lip Sync.

7) Lip Sync
Use when the creator mouths lyrics, sings along, performs to lyrics, or acts out audio/dialogue without dance being dominant.
Examples: person singing along to the track -> Lip Sync; dramatic mouthing of lyrics -> Lip Sync.

8) Lyrics
Use when original song lyrics are visibly displayed on screen, lyric edit style, karaoke-style text, or the post is mainly lyric-focused.
Do NOT use Lyrics merely because the song has lyrics; the lyrics must be visually or captionally central.

9) Lyrics Translation
Use when lyrics are translated into another language, bilingual lyric subtitles are shown, or translation is the main purpose.

10) Quotes
Use when the post mainly presents a quote, short saying, text quote, motivational quote, emotional quote, or aesthetic quote card.
Quotes usually present a phrase/line as the content itself.
Different from Reflection: Quotes = sharing a quoted line; Reflection = personal introspection or life lesson.

11) Reflection
Use for personal thoughts, emotional introspection, self-growth, life lessons, self-acceptance, gratitude, sadness, healing, personal realization, or reflective storytelling.
Different from Quotes: Reflection feels like personal meaning or internal emotion, not just a standalone quote.

12) Comedy
Use when the main purpose is humour: funny skit, joke, meme, prank, humorous situation, comedic acting, exaggerated reaction.
If the format is a POV but the main purpose is humour, Comedy can be used with POV.

13) POV
Use when the post presents a first-person scenario, roleplay, or "POV:" setup. It often acts out a situation from the viewer's perspective.
If it is clearly funny, use Comedy, POV. If it is romantic, use Relationship, POV.

14) Media/Infotainment
Use for educational, informative, explanatory, news, tutorial, DIY, tips, product recommendation, review, facts, horoscope/forecast, commentary, or how-to content.
Examples: news article post -> Media/Infotainment; Valentine bracelet idea/tutorial -> Media/Infotainment; homemade Raya cake idea -> Media/Infotainment; digicam recommendation -> Media/Infotainment.

15) Travel
Use when the main visual focus is destination, scenery, travel experience, beach, mountains, city, vacation, tourism, trip vlog, or landscape.
If the visual focus is travel even with emotional caption, use Travel.

16) Gaming
Use when the main focus is gameplay, game screenshots, game characters, game edits, gaming clips, game fan content, or gaming interface.
Examples: Minecraft clip -> Gaming; Genshin/game character edit -> Gaming; Racing Master/Omori game edit -> Gaming.

17) Fitness
Use for gym, workout, exercise, sports training, bodybuilding, flexing muscles, showing physique, fitness transformation, or active training.

18) Cover
Use when the creator performs or sings a cover version of the song, plays an instrument, or records their own musical performance.

19) Remix
Use ONLY when the audio is clearly remixed, sped up, slowed, mashup, DJ edit, or alternate audio version. Never use Remix for visual editing or outfit transitions.

20) Celebrity Edits
Use for fan edits, montages, or compilations of celebrities, idols, public figures, athletes, K-pop idols, actors, performers, or public personalities.
KR/K-pop examples often belong here unless dance performance by creator is the main focus.

21) Movie/Tv/Drama Edits
Use for edits or scenes centered on movies, TV shows, dramas, series, fictional scenes, or drama characters.
If the edit is about a real celebrity/idol, use Celebrity Edits. If it is about a drama/movie scene, use Movie/Tv/Drama Edits.

=== DECISION PRIORITY ===
1. If isSlideshow=true -> include Carousel.
2. Clothing/outfit/styling focus -> Fashion.
3. Makeup/skincare/hair/nails/cosmetics focus -> Beauty.
4. Romantic love/partner/longing focus -> Relationship.
5. Everyday routine/aesthetic life with no strong romance -> Slice of Life.
6. Dance/choreography is dominant -> Dance.
7. Mouthing/singing lyrics is dominant -> Lip Sync.
8. Visible lyrics are central -> Lyrics; translated lyrics -> Lyrics Translation.
9. Informational/tutorial/news/review/DIY -> Media/Infotainment.
10. Celebrity/idol/public figure montage -> Celebrity Edits.
11. Movie/TV/drama scene edit -> Movie/Tv/Drama Edits.
...
REASONING: one short sentence explaining why the chosen Creative Type fits.

=== OUTPUT FORMAT (JSON only) ===
{{"narrative": "<short theme phrase or NA>", "creative_type": ["<label1>"], "content_details": "<sentence>", "confidence": <float>, "reasoning": "<sentence>"}}"""
